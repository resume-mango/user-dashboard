...
import * as progressTrackerQueries from '../../queries/progressTrackerQueries'
import { progressTrackerData } from '../../__mocks__/progressTracker'
import TestingWrapper from '../../__mocks__/TestingWrapper'
import { countsData } from '../../__mocks__/counts'

describe('Progress Tracker Context', () => {
  const TestWrapper = () => {
    const {
      showModal,
      setShowModal,
  ...
  const getAllTrackersSpy = jest.spyOn(progressTrackerQueries, 'getAllTrackers')
  const dashCountsFetcherSpy = jest.spyOn(dashQueries, 'dashCountsFetcher')
  const deleteProgressTrackerQuerySpy = jest.spyOn(
    progressTrackerQueries,
  ...
  beforeEach(() => {
    getAllTrackersSpy.mockReturnValue({
  ...
  test('should render successfully', async () => {
    ...
    deleteProgressTrackerQuerySpy.mockReturnValue({
      mutate,
    } as any)
    ...
    expect(data.textContent).toEqual(
      `Data: ${JSON.stringify(progressTrackerData)}`
    )
